		...
		<div class="col-sm-12 col-md-5">
			<div class="row">
				<div class="col-md-6">
					...
					<select class="form-control select2" <?= $_SESSION['ideyetu_country'] == "Congo"?"id='drc_classes'":"" ?> name="<?= $_SESSION['ideyetu_country'] != "cd"?'class':'drc_class' ?>" required>
				...
				<div class="col-sm-6">
					<label><?= lang("app.selectSemester"); ?> :</label>
					<select class="select2" id="select_semester" name="semester_id" required>
						<option disabled selected><?= lang("app.selectSemester"); ?> </option>
						<?php
						if(count($semesters->semesters) > 0){
							foreach($semesters->semesters AS $semester){
								if(is_null($semester->enabled)){
									continue;
								}
								?>
								<option value="<?= $semester->enabled->id ?>"><?= $semester->name ?></option>
								<?php
							}
						}
						?>
			...
			<div class="row">
				<div class="col-sm-12" id="assessment_container" style="max-height: 380px; overflow-y: auto; margin-top: 5px;">

				</div>
			</div>
		</div>
		<div class="col-sm-12 col-md-7" id="marks_entry_container" style="max-height: 445px; overflow-y: auto; margin-top: 5px;">
		</div>
	</div>
</div> <?php /*
...
<script>
	$(function () {
		...
		$("#select_semester").on("change", function(e){
			...
			$("#assessment_container").html("Loading....");
			$("#assessment_container").load("<?= base_url("get_assessments") ?>/" + $("#drc_classes").val() + "/" + $(this).val(), function(){
				//The Loading process completed successfuly
			});
		});
	})
